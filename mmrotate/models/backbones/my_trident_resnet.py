...
class TridentConv(BaseModule):
    ...
    def forward(self, inputs):
        ...
        #         # 0->0 1->2 2->4 w2
        #         # 0->0 1->3 2->6 w3
        #         self.w2[:,:,i*2,j*2] = self.w1[:,:,i,j]
        #         self.w3[:,:,i*3,j*3] = self.w1[:,:,i,j]
        '''
        # 验证w2可以向w1传导梯度
        out1 = torch.zeros_like(out1)
        out2 = F.conv2d(inputs[0], self.w2, ...)
        out3 = torch.zeros_like(out3)
        print(self.w1)
        '''
...
class MyTridentResNet(ResNet):
    ...
    def __init__(self, depth, num_branch, test_branch_idx, trident_dilations,
        ...
        super(MyTridentResNet, self).__init__(depth, **kwargs)
