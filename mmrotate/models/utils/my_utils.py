...
def show_features(xi: torch.Tensor, name):
    x = xi.cpu().data.squeeze(0)
    # print(x.shape)
    ...
    # plt.figure()
    # plt.imshow(x)
    # plt.savefig("./workdirs/feature.jpg", dpi=300)
    # # cv2.imwrite('./workdirs/feature.jpg',x)

    # 创建一个10x10的子图，每个子图展示一个通道的特征图
    ...
    for i in range(10):
        for j in range(10):
            channel = i * 10 + j  # 计算当前子图对应的通道索引
            axs[i, j].imshow(x[:, :, channel], cmap='gray')
            axs[i, j].set_title(f'Channel {channel}')  # 设置子图标题
...
def draw_feature_map1(features, img_path, save_dir='./work_dirs/feature_map/', name=None):
    ...
    for featuremap in features:  # 循环遍历
        ...
        cv2.imwrite(os.path.join(save_dir, name + str(i) + '.png'),
                    superimposed_img)  # superimposed_img：保存的是叠加在原图上的图，也可以保存过程中其他的自己看看
        cv2.imwrite(os.path.join(save_dir, "heatmap" + str(i) + '.png'),
                    heatmap)  # superimposed_img：保存的是叠加在原图上的图，也可以保存过程中其他的自己看看
