...
class FPNSE_Conv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        w2_center = F.avg_pool2d(w3, kernel_size=(3, 3), stride=1)  # [256, 256, 1, 1]
        w2 = self.w[self.out_channels:-self.out_channels, :, :, :]
        w2_p1 = w2[:, :, :1, :]
        w2_p2 = w2[:, :, -1:, :]
        w2_p3 = w2[:, :, 1:-1, :1]
        w2_p4 = w2[:, :, 1:-1, -1:]
        w2_o1 = torch.cat([w2_p3, w2_center, w2_p4], dim=3)
        w2_o2 = torch.cat([w2_p1, w2_o1, w2_p2], dim=2)  # [256, 256, 3, 3]

        # w1 center是对w2进行pooling
        w1_center = F.avg_pool2d(w2_o2, kernel_size=(3, 3), stride=1)  # [256, 256, 1, 1]
