...
class FPNSE31(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.start_level, self.backbone_end_level):  # range(0, 4)
            ...
            # if i < 3:
            fpnse_conv = FPNSE_Conv(out_channels, out_channels, kernel_size=3)
...
class FPNSE_Conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, padding=0, dilation=1):
        ...
        self.w = nn.Parameter(torch.randn(2 * out_channels, in_channels, self.kernel_size, self.kernel_size),
                              requires_grad=True)
        self.bias = nn.Parameter(torch.empty(2 * out_channels), requires_grad=True)
    ...
    def forward(self, inputs):
        ...
        w2 = self.w[-self.out_channels:, :, :, :]
        # w1 center是对w2进行pooling
        w1_center = F.avg_pool2d(w2, kernel_size=(3, 3), stride=1)  # [256, 256, 1, 1]
        ...
        w1_p3 = w1[:, :, 1:-1, :1]
        w1_p4 = w1[:, :, 1:-1, -1:]
        w1_o1 = torch.cat([w1_p3, w1_center, w1_p4], dim=3)
        # print("w1_o1.shape:", w1_o1.shape)
        w1_o2 = torch.cat([w1_p1, w1_o1, w1_p2], dim=2)  # [256, 256, 3, 3]
        # print("w1_o2.shape:", w1_o2.shape)
        w = torch.cat([w1_o2, w2], dim=0)
