...
class AtrousSE(BaseModule):
    def __init__(self, in_channels, out_channels, kernel_size=3, strides=(1, 1, 1), dilations=(1, 2, 3),
        ...
        # 定义卷积层
        self.conv3d_layer = torch.nn.Conv3d(in_channels=self.M,
                                            out_channels=d,
                                            kernel_size=(3, 3, 3))
    ...
    def forward(self, inputs):
        ...
        feaU = self.conv3d_layer(feas)  # [2, d, 256, 256, 256]
        feaS = feaU.mean(-1).mean(-1).mean(-1)  # [2, d]
        feaZ = self.fcZ(feaS)  # [2, 3]
        # *100
        att_alpha = self.softmax(feaZ)
        att_alpha = att_alpha.reshape(att_alpha.shape[0], att_alpha.shape[1], 1, 1, 1)
        feaV = feas * att_alpha
        return feaV
